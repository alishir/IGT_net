...
poster_clusters <- function(alpha = 0.3)
{
	...
	lon = learn_or_not;
	png('/tmp/outcome_com_random.png', bg = "transparent");
	par(cex.axis = 2.5, cex.lab = 2.5,
		oma = c(1.5,1,0,0), mar = c(1.5,5,3,2));
	matplot(t(lon$outcome$com$random), type = 'l', pch = 1:nrow(lon$outcome$com$random), lwd=4, ylab = "Performance", xlab = "Trial",
			main = "Random Subjects");
	dev.off();

	png('/tmp/outcome_com_learn.png', bg = "transparent");
	par(cex.axis = 2.5, cex.lab = 2.5,
		oma = c(1.5,1,0,0), mar = c(1.5,5,3,2));
	matplot(t(lon$outcome$com$learn), type = 'l', pch = 1:nrow(lon$outcome$com$learn), lwd=4, ylab = "Performance", xlab = "Trial",
			main = "Subjects who learned the IGT");
	dev.off();

	png('/tmp/outcome_com_unlearn.png', bg = "transparent");
	par(cex.axis = 2.5, cex.lab = 2.5,
		oma = c(1.5,1,0,0), mar = c(1.5,5,3,2));
	matplot(t(lon$outcome$com$unlearn), type = 'l', pch = 1:nrow(lon$outcome$com$unlearn), lwd=4, ylab = "Performance", xlab = "Trial",
			main = "Subjects who Didn't learn the IGT");
	dev.off();
