...
while itr < max_itr			% iteration of card selection by subject
	...
	[selected_deck, resp_time] = read_response_box(resp_handle);
	disp(selected_deck);
	...
	selected_deck = shuffle_decks(selected_deck);
	ind = selected_deck;
...
if (punish == 0)
	...
	Screen('DrawText', wPtr, msg_text2, text_w - offset_w, text_h + offset, black, gray);
	Screen('DrawText', wPtr, sprintf('  %d', punish), text_w + offset_w, text_h + offset, black, gray);		% other numbers are left aligned
end
Screen('Flip', wPtr);
tic
while toc < 1.25 end;
...
function [key, when] = read_response_box(resp_handle)
